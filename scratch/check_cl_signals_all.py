...
url = f"{SUPABASE_URL}/rest/v1/signals?select=symbol,exchange,created_at,outcome,status,metadata&order=created_at.desc&limit=10"
req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req) as response:
        signals = json.loads(response.read().decode())
        print("Last 10 signals in DB:")
        for s in signals:
            print(f"Symbol: {s['symbol']} | Exch: {s['exchange']} | CreatedAt: {s['created_at']} | Outcome: {s['outcome']} | Status: {s['status']}")
