...
- **Mandatory Triple-Binding on Webhook Updates (`TradeFill`, `TradeClose`, `TrailingSLUpdate`)**:
  ...
  3. **Time-Window Binding (MANDATORY)**: Candidate search windows for trade fills and updates must be strictly bound to the immediate setup session (**maximum 4 hours**, never an indiscriminate 24-hour sweep). An order executing at 21:45 PM must **NEVER** match a morning order from 10:30 AM.
- **Safe Fall-Through to INSERT**: When a `TradeFill` webhook arrives and no open limit order matches on **Direction + Price + Time**, the backend must **NEVER** guess or mutate an unrelated existing trade. It must gracefully fall through to the insert routine, recording the new trade cleanly as its own independent active position.
