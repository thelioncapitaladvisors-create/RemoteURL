...
- **Cross-Origin Iframe Embedding Security**: Modified `_headers` on the live Netlify deployment to completely strip the `X-Frame-Options: SAMEORIGIN` block. This globally opens the iframe gateway, explicitly permitting external Capacitor/Expo environments (like the live Mobile App) to seamlessly embed and auto-refresh the VectorBT tearsheet without triggering browser origin policy violations.
