...
- **Pre-Defined Limit Level Binding & Deterministic Exit Verification**:
  - **Immutable Pre-Defined Limit Levels**: At signal genesis (limit order creation), all structural levels (`entry_price`, `stop_loss`, `tp1`, `tp2`, `tp3`, `tp4`) are mathematically calculated by the indicator engine and permanently locked into the Supabase database columns on that trade row.
  - **Ground Truth Exit Verification**: When a trade exits, the system never generates or guesses arbitrary levels. Comparing `exit_price` against `s.tp1`, `s.tp2`, `s.tp3`, and `s.tp4` is a direct binding check back to the trade's own pre-defined database columns created at inception.
  - **Zero Heuristics & Deterministic Resolution**: If `exit_price` equals `1.15686`, matching against `s.tp3 (1.15686)` directly links the executed exit to the original pre-defined `TP3` limit column. This removes all external string ambiguities (e.g. generic `TARGET` webhooks or trailing stop triggers) and guarantees that the trade card, badges, and analytics reflect the exact pre-defined target tier with 100% mathematical fidelity.
...
- **Weekend Scan Persistence & Trading Day Boundaries**: Market close calculators (`getLastMarketCloseTs`) across Web (`dashboard.html`, `trade-metrics.js`) and Mobile (`page.tsx`) must be calendar- and market-aware. On non-trading days (Saturday and Sunday) and Monday morning before session close, the last market close MUST resolve back to Friday's session close (or Saturday 04:00 IST for NYMEX/Forex/World). `isScanValidForMarket` must never invalidate Friday scans over the weekend. Furthermore, `cron-eod-close.js` is strictly forbidden from resetting `pivotboss_scans.scan_data` on weekends (Saturday and Sunday), and weekday reset requires >72 hours of staleness.
...
## Trailing SL Webhook Payload Key Architecture
- **Canonical Pine Script Trailing Stop Key (`slLevel`)**: Pine Script's `sendTrailingSLAlert` transmits the trailed stop loss under the JSON key `"slLevel"` (and the message under `"tradeMessage"`).
- **Backend Multi-Key Resolution**: All webhook handlers (`process-webhook-background.js` and `Tv-Alert-Mobile/src/app/api/webhook/route.ts`) MUST resolve trailing stop loss levels defensively across all key variations:
  ```javascript
...
- **Strict Position Independence**: A `LONG` signal must **NEVER** overwrite, mutate, fill, trailing-stop-update, or close an active `SHORT` signal (and vice versa). Each trade direction represents an entirely independent position lifecycle.
- **Mandatory Triple-Binding on Webhook Updates (`TradeFill`, `TradeClose`, `TrailingSLUpdate`)**:
  1. **Direction Binding (MANDATORY)**: Direction matching must definitively resolve `LONG` vs `SHORT` by evaluating whether `action` or `type` contains `'LONG'`/`'BUY'` or `'SHORT'`/`'SELL'`. Strict string equality `=== 'LONG'` is prohibited because payloads transmit composite strategy strings (e.g. `LONG LIGHTNING`, `SHORT MISSILE`, `LONG SCALP`). If direction cannot be definitively established, fallback matching is aborted.
  2. **Entry Price Binding (MANDATORY)**: When an incoming webhook provides an entry price (`entryPrice`, `entry`), the candidate record in Supabase must match the price within a strict **±0.2% tolerance**. An order filled at `8,624.00` must **NEVER** match an order placed at `8,548.00`.
- **Preservation of Trade Lifespan (No Arbitrary Time Cutoffs)**: A valid executed trade can run for 5–6 hours, all day, or until its natural EOD exit. The backend must NEVER impose artificial time limits (such as an arbitrary 4-hour cutoff) on `TradeClose`, `TrailingSLUpdate`, `TradeFill`, or `TradeUpdate`. Trades remain active and valid until closed by their explicit exit conditions.
- **Safe Fall-Through to INSERT**: When a `TradeFill` webhook arrives and no open limit order matches on **Direction + Price**, the backend must **NEVER** guess or mutate an unrelated existing trade. It must gracefully fall through to the insert routine, recording the new trade cleanly as its own independent active position.

## Automatic & Immediate Deletion of Invalidated/Expired Limit Orders
- **Zero-Ghost Policy**: Whenever an unexecuted limit order is invalidated or expired in Pine Script (`status` or `trigger` contains `INVALID`, `CANCEL`, or `EXPIRE`), the webhook backend MUST immediately and permanently delete the record from Supabase via `.delete().eq('id', activeSignal.id)`.
- **Complete Removal from Future Consideration**: An invalidated or expired limit order represents an unfilled setup that no longer exists in the market. It must be completely expunged from the database so it can NEVER linger, pollute active signals, or be mistakenly matched or mutated by any subsequent trades (such as a later session trade on the same symbol).
- **No Orphaned Open Limits**: Unexecuted limit orders that do not fill within their market session must never survive across sessions to collide with subsequent trade fills.

## Original Trading Logic Inviolability (Zero Modifications to Entry, Exit, or SL)
...
- **Weekly Cron Execution**: Scheduled aggregation in `TLCS_Website_Deploy/netlify/functions/cron-weekly-logs.js` and `.github/workflows/weekly-performance-cron.yml` is anchored to **`30 18 * * 0`** (Sunday 18:30:00 UTC = Sunday 24:00:00 / Monday 00:00:00 IST), automatically finalizing and publishing the completed calendar week's performance edge to `weekly_performance_logs` every Sunday at midnight IST without fail, with a pre-close sweep for traditional markets on Saturday (`30 18 * * 6`).
