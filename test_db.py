...
url = f"{SUPABASE_URL}/rest/v1/signals?select=*&created_at=gte.2026-06-21T18:30:00.000Z&order=created_at.desc&limit=100"

req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req) as response:
        data = json.loads(response.read().decode())
        print(f"Fetched {len(data)} total signals.")
        for s in data:
            if s.get('exit_price'):
                print(s)
