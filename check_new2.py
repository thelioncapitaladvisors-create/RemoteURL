...
url = f"{SUPABASE_URL}/rest/v1/signals?select=id,symbol,type,status,created_at&order=created_at.desc&limit=15"
req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req) as response:
        data = json.loads(response.read().decode())
        for s in data:
            print(f"{s['symbol']} {s['type']} -> {s['status']} | {s['created_at']}")
