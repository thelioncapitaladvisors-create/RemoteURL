...
url = f"{SUPABASE_URL}/rest/v1/signals?select=symbol,status,updated_at&symbol=in.(HSI,EURJPY,GBPUSD)&order=updated_at.desc&limit=5"
req = urllib.request.Request(url, headers=headers)
try:
    with urllib.request.urlopen(req) as response:
        data = json.loads(response.read().decode())
        for s in data:
            print(f"{s['symbol']} -> {s['status']} | {s.get('updated_at', 'None')}")
