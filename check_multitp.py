...
url = f"{SUPABASE_URL}/rest/v1/signals?select=id,symbol,type,status,outcome,created_at,exit_at&order=created_at.desc"
...
try:
    with urllib.request.urlopen(req) as response:
        ...
        for s in data[:20]:
            print(f"{s['symbol']} {s['type']} -> Status: {s['status']} | Outcome: {s['outcome']} | Created: {s['created_at']}")
