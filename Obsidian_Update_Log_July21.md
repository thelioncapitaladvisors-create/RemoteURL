...
- **Fix F (Exit Lookup Engine Upgrade)**: Fixed 4 real bugs in webhook exit signal lookup (`process-webhook-background.js` and `route.ts`): (1) 10-digit Unix timestamp parsing, (2) `entryPrice` tolerance binder, (3) ±60s window expansion, and (4) FIFO fallback. `close_stale_trades.py` remains strictly for stale trades > 24 hours old.
