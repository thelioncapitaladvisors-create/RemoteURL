...
- **The Issue**: Live Market Executions trigger initial webhooks with `status: "OPEN"` and `trigger: null`. Because these are the *first* inserts into the database, they lack an `updated_at` timestamp. The frontend previously bucketed any open trade lacking an `updated_at` as an unexecuted limit order, causing live market executions to mistakenly render as `ACTIVE LIMITS` and completely disappear from the `LIVE` filter view.
- **The Fix**: Rewrote the parser logic in both the mobile app (`page.tsx`) and the web dashboard (`trade-metrics.js`). The engine now safely overrides the missing timestamp and forcefully categorizes any trade that does NOT explicitly contain `LIMIT` in its status or trigger as a valid `LIVE TRADE`, completely restoring the live data feeds.

## 5. System Reliability & Webhook Hardening
- **Pine Script JSON Sanitization (`process-webhook-background.js`)**: TradingView webhooks often contain unescaped control characters (`\n`, `\t`, `\r`, `\f`) within multi-line string fields (like `opening_bias`). The backend webhook MUST globally intercept and sanitize all literal control characters using `replace(/[\n\r\t\v\f\b]+/g, ' ')` before calling `JSON.parse()` to prevent fatal `400 Invalid JSON` parse errors.
- **Global DB Fetch Timeouts (`process-webhook-background.js`)**: Netlify Functions enforce a strict 10-second execution limit. All Supabase JS clients running on Netlify MUST inject a global fetch handler with `AbortSignal.timeout(5000)`. This forces the Supabase connection to abort early during transient DB latency spikes, allowing internal error handling to gracefully execute and exit instead of crashing the serverless container with a `502/504 Bad Gateway`.
- **Graceful Payment Degradation (`razorpay-webhook.js`)**: Wrapped DB RPC lookups (`get_user_id_by_email`) in a strict `try/catch`. If the DB fetch times out, the script intentionally returns a `200 OK` directly to Razorpay. Returning a `500` error causes Razorpay to infinitely retry the webhook every 20 minutes and violently flood the system with duplicate alerts.
- **Mobile Network Resilience (`Tv-Alert-Mobile/src/lib/supabase.ts`)**: Handled 4G/5G packet loss by injecting a custom Exponential Backoff Retry Engine directly into the Supabase global fetch handler. It silently intercepts 500-level errors and fetch failures, retrying up to 3 times (with 1s, 2s, 4s expanding delays) and enforcing a 10s maximum timeout per attempt to eliminate blank screens.
