...
async function fix() {
  const { data, error } = await supabase
    .from('signals')
    .select('*')
    .in('status', ['Closed', 'Closed: TP', 'Closed: SL', 'Force Closed'])
  ...
  for (const s of data) {
    ...
    // Check if the current r_multiple in DB has the WRONG sign compared to the true math!
    // E.g., if DB r_multiple is negative, but true math is positive!
